...
void listSavedEntries() {
    ...
    while (file.available()) {
        ...
        // Check if line indicates a new entry (starts with "Time:")
        if (line.startsWith("Time:")) {
            // If we have accumulated text for a previous entry, save it
            if (entryText.length() > 0) {
                LogEntry entry;
                entry.text = entryText;
                entry.timestamp = parseTimestamp(entryText);
                parsedLogEntries.push_back(entry);
                DEBUG_PRINTF("Parsed entry:\n%s(Timestamp: %ld)\n", entry.text.c_str(), entry.timestamp);
                entryText = "";
            }
            newEntry = true;
        }

        // Append non-empty lines to the current entry
        if (line.length() > 0) {
            entryText += line + "\n";
        }
    }

    // Handle the last entry if file doesn't end with a blank line
    if (entryText.length() > 0) {
        LogEntry entry;
        entry.text = entryText;
        entry.timestamp = parseTimestamp(entryText);
    ...
    DEBUG_PRINT("Entries sorted by timestamp");
