...
void setup() {
    ...
    // Initialize display buffer with both buffers for double buffering
    ...
    // Load saved networks (for backward compatibility)
    loadSavedNetworks();
...
void createMainMenu() {
    ...
    // Create a 3x2 grid layout for the buttons
    // Calculate button dimensions based on screen size
    const int BTN_MARGIN = 10;
    const int BTN_WIDTH = (SCREEN_WIDTH - (3 * BTN_MARGIN)) / 2;
    const int BTN_HEIGHT = 60;
    const int START_Y = 50; // Starting Y position after header
    
    // Button 1: New Entry (Top Left)
    lv_obj_t* btn_new = lv_btn_create(mainScreen);
    lv_obj_set_size(btn_new, BTN_WIDTH, BTN_HEIGHT);
    lv_obj_set_pos(btn_new, BTN_MARGIN, START_Y);
    ...
    // Button 2: Wi-Fi Settings (Top Right)
    lv_obj_t* btn_wifi = lv_btn_create(mainScreen);
    lv_obj_set_size(btn_wifi, BTN_WIDTH, BTN_HEIGHT);
    lv_obj_set_pos(btn_wifi, BTN_WIDTH + (2 * BTN_MARGIN), START_Y);
    ...
    // Button 3: View Logs (Middle Left)
    lv_obj_t* btn_logs = lv_btn_create(mainScreen);
    lv_obj_set_size(btn_logs, BTN_WIDTH, BTN_HEIGHT);
    lv_obj_set_pos(btn_logs, BTN_MARGIN, START_Y + BTN_HEIGHT + BTN_MARGIN);
    ...
    // Button 4: Settings (Middle Right)
    lv_obj_t* btn_settings = lv_btn_create(mainScreen);
    lv_obj_set_size(btn_settings, BTN_WIDTH, BTN_HEIGHT);
    lv_obj_set_pos(btn_settings, BTN_WIDTH + (2 * BTN_MARGIN), START_Y + BTN_HEIGHT + BTN_MARGIN);
    ...
    // Button 5: Placeholder 1 (Bottom Left)
    lv_obj_t* btn_placeholder1 = lv_btn_create(mainScreen);
    lv_obj_set_size(btn_placeholder1, BTN_WIDTH, BTN_HEIGHT);
    lv_obj_set_pos(btn_placeholder1, BTN_MARGIN, START_Y + (2 * (BTN_HEIGHT + BTN_MARGIN)));
    lv_obj_add_style(btn_placeholder1, &style_btn, 0);
    lv_obj_add_style(btn_placeholder1, &style_btn_pressed, LV_STATE_PRESSED);
    lv_obj_t* label_placeholder1 = lv_label_create(btn_placeholder1);
    lv_label_set_text(label_placeholder1, "Future Feature 1");
    lv_obj_center(label_placeholder1);
    lv_obj_add_event_cb(btn_placeholder1, [](lv_event_t* e) { 
        updateStatus("Feature coming soon!", 0xFFAA00);
    }, LV_EVENT_CLICKED, NULL);

    // Button 6: Placeholder 2 (Bottom Right)
    lv_obj_t* btn_placeholder2 = lv_btn_create(mainScreen);
    lv_obj_set_size(btn_placeholder2, BTN_WIDTH, BTN_HEIGHT);
    lv_obj_set_pos(btn_placeholder2, BTN_WIDTH + (2 * BTN_MARGIN), START_Y + (2 * (BTN_HEIGHT + BTN_MARGIN)));
    lv_obj_add_style(btn_placeholder2, &style_btn, 0);
    lv_obj_add_style(btn_placeholder2, &style_btn_pressed, LV_STATE_PRESSED);
    lv_obj_t* label_placeholder2 = lv_label_create(btn_placeholder2);
    lv_label_set_text(label_placeholder2, "Future Feature 2");
    lv_obj_center(label_placeholder2);
    lv_obj_add_event_cb(btn_placeholder2, [](lv_event_t* e) { 
        updateStatus("Feature coming soon!", 0xFFAA00);
    }, LV_EVENT_CLICKED, NULL);
...
void scan_timer_callback(lv_timer_t* timer) {
    ...
    // Check scan status using WiFiManager
    int scanStatus = wifiManager.getScanStatus();
...
void network_batch_timer_callback(lv_timer_t* timer) {
    if (wifi_list == nullptr || !lv_obj_is_valid(wifi_list)) {
    ...
    for (int i = startIdx; i < endIdx; ++i) {
        // Get network info from WiFiManager
        NetworkInfo network = wifiManager.getScanResult(i);
        ...
        String security = (wifiManager.getEncryptionType(i) == WIFI_AUTH_OPEN) ? "" : "*";
...
void connectToWiFi() {
    ...
    // Use WiFiManager to connect to the network
    bool connected = wifiManager.connectToNetwork(selected_ssid, selected_password);
...
void createWiFiManagerScreen() {
    ...
    const std::vector<NetworkInfo>& networks = wifiManager.getSavedNetworks();
    ...
    if (networks.empty()) {
        ...
        for (size_t i = 0; i < networks.size(); i++) {
            ...
            String status = (wifiManager.isConnected() && WiFi.SSID() == networks[i].ssid) 
                ? " (Connected)" : "";
            String ssid_text = networks[i].ssid + status;
            ...
            lv_obj_add_event_cb(connect_btn, [](lv_event_t* e) {
                ...
                // Attempt connection using WiFiManager
                bool connected = wifiManager.connectToNetwork(networks[idx].ssid.c_str(), networks[idx].password.c_str());
                ...
                // Show result
                const char* msg = connected ? "Connected successfully!" : "Failed to connect";
            ...
            lv_obj_add_event_cb(disconnect_btn, [](lv_event_t* e) {
                ...
                if (wifiManager.isConnected() && WiFi.SSID() == networks[idx].ssid) {
